...
public class ArtistService {
	...
	public String insert(Map<String, Object> paramMap){
		...
		if(profile.getSize() > 0) {
			...
			String filename = "profile_" + date.format(formatter3) + "_" + folder.listFiles().length + "." + ext;
	...
	public String update(Map<String, Object> paramMap) {
		...
		// 작가수정
		ArtistDTO artist = new ArtistDTO();
		artist.setArtistNo((String)paramMap.get("artistNo"));
		artist.setNameKor((String)paramMap.get("nameKor"));	// nameKor
		artist.setNameEng((String)paramMap.get("nameEng")); // nameEng
		artist.setBirthday((Date)paramMap.get("birthday")); // birthday
		String[] phoneArr = (String[])paramMap.get("phone"); 
		...
		artist.setOpenYn((boolean)paramMap.get("openY")?"Y":"N");
		System.out.println("작가등록:" + artist.toString());
		boolean isInsert = dao.updateArtist(artist);
		if(!isInsert) { // DB 작가등록 실패
			returnMessage.append("<script>\n");
			returnMessage.append("alert('ERROR:작가 수정 실패');\n");
			returnMessage.append("history.back();\n");
			returnMessage.append("</script>");
			return returnMessage.toString();
		}
		...
		// 작가 주소 전체 삭제
		dao.deleteArtistAddress((String)paramMap.get("artistNo"));
		// 작가 주소 등록
		@SuppressWarnings("unchecked")
		List<String[]> addrList = (List<String[]>)paramMap.get("addrList");
		for (String[] addrs : addrList) {
			ArtistAddressDTO address = new ArtistAddressDTO();
			address.setArtistNo((String)paramMap.get("artistNo"));
			address.setAddressName(addrs[0]);
			address.setAddressn(addrs[1]);
			address.setAddress1(addrs[2]);
			if(addrs.length-1 >= 3) address.setAddress2(addrs[3]);
			dao.insertArtistAddress(address);
		}
		
		returnMessage.append("<script>\n");
		returnMessage.append("alert('작가정보가 수정되었습니다.');");
		returnMessage.append("location.replace('" + ExhibitionConfig.REFERER_URL + "/admin/artist-view.html?artistNo=" + (String)paramMap.get("artistNo") + "');\n");
		returnMessage.append("</script>");
		return returnMessage.toString();
	}
	...
	public Map<String, Object> selectOne(String artistNo) {
		...
		List<ArtistAddressDTO> addressList = dao.selectArtistAddress(artistNo);
		
		if(dto == null) {
			resultMap.put("result", false);
			resultMap.put("artistData", null);
			resultMap.put("addressData", null);
		} else {
			resultMap.put("result", true);
			resultMap.put("artistData", dto);
			resultMap.put("addressData", addressList);
		}
	...
	public List<ExhibitionDTO> artExhibition(String artistNo) {
		return dao.artExhibition(artistNo);
	}
